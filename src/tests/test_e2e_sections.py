...
class TestE2eSections(TestE2eBase):
    def test_section_appears_once(self):
        ...
        """
        self._add_cheatsheet_to_db("test_snippet", "test_section_1")
        self._add_cheatsheet_to_db("test_snippet", "test_section_2")
        self._add_cheatsheet_to_db("test_snippet", "test_section_3")
        self._add_cheatsheet_to_db("test_snippet", "test_section_1")
        self._add_cheatsheet_to_db("test_snippet", "test_section_2")

        response = requests.get(URL.INDEX.value)
        self._compare_num_cheatsheets(response, 5)
    ...
    def test_no_section_first(self):
        self._add_cheatsheet("test_snippet_3", "test_section_1")
        # special case - test that " " is treated as ""
        # so when sorting, it should come before the next cheatsheet (with "" section)
        self._add_cheatsheet("test_snippet_1", " ")
        self._add_cheatsheet("test_snippet_2", "")
    ...
    def test_section_slash_separator(self):
        self._add_cheatsheet_to_db("test_snippet", "Test_section/sub_section")
        self._add_cheatsheet_to_db("test_snippet", "test_section / sub_section")
        self._add_cheatsheet_to_db("test_snippet", "test_section/ sub_section")
        self._add_cheatsheet_to_db("test_snippet", "test_section /sub_section")
        response = requests.get(URL.INDEX.value)
